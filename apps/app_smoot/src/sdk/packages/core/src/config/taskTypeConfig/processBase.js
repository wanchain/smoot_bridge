...
class ProcessBase {
  ...
  async getConvertInfoForCheck(stepData) {
    return {
      eventSc: null, // only for event which not emit by tx.to
      txEventTopics: null,
  ...
  async sendTransactionData(stepData, txData, wallet) {
    ...
    try {
      ...
      let {eventSc, txEventTopics, convertCheckInfo} = await this.getConvertInfoForCheck(stepData);
      let txCheckInfo = {from: txData.from, to: txData.to, eventSc, topics: txEventTopics, fromBlock, input: "", nonce: undefined, nonceBlock: 0};
